...
if [ ! -f "$install_location/bin/adi_colorimeter" ] || [ ! -f "$install_location/lib/adi_colorimeter/capture.so" ] || \
    [ ! -f "$install_location/share/applications/adi_colorimeter.glade" ] || \
    [ ! -f "$install_location/share/polkit-1/actions/com.analogdevicesinc.adi_colorimeter.policy" ] || \
    [ ! -f "$install_location/share/applications/adi_colorimeter.desktop" ] || \
    [ ! -f "$install_location/share/icons/hicolor/16x16/apps/adi_colorimeter.png" ] || \
    [ ! -f "$install_location/share/icons/hicolor/32x32/apps/adi_colorimeter.png" ] || \
    [ ! -f "$install_location/share/icons/hicolor/64x64/apps/adi_colorimeter.png" ]; then
